...
plot.wf_diagnostics <- function(x, ...) {
  ...
  do.call(
    graphics::dotchart,
    .wf_plot_args(
      list(x = table$deff, labels = table$group, xlab = "design effect"),
      dots
    )
  )
...
#' @return Invisibly returns `x`.
#' @export
plot.wf_blend_result <- function(x, ...) {
...
#' @return Invisibly returns `x`.
#' @export
plot.wf_propensity_weights <- function(x, ...) {
    ...
    do.call(
      ...
      .wf_plot_args(
        list(
          ...
          main = "Propensity overlap",
          xlab = "online-membership propensity",
          ylab = "density",
          col = "#0072B2",
          lwd = 2
        ),
