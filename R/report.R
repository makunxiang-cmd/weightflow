...
#' @keywords internal
#' @noRd
.wf_report_markdown_table <- function(x) {
...
#' @keywords internal
#' @noRd
.wf_report_html_table <- function(x) {
  if (!is.data.frame(x) || ncol(x) == 0) {
    return("<p><em>No rows available.</em></p>")
  }
  head <- paste0(
    "<thead><tr>",
    paste0("<th>", .wf_report_html_escape(names(x)), "</th>", collapse = ""),
    "</tr></thead>"
  )
  body <- if (nrow(x) == 0) {
    "<tbody></tbody>"
...
#' @keywords internal
#' @noRd
.wf_report_manager_weights <- function(diagnostics) {
  light <- ifelse(
  ...
  action <- c(
    green = "publishable separately",
    yellow = "usable with caveats; prefer aggregated publication",
    red = "do not publish separately; merge or remediate"
  )[light]
  cbind(
    diagnostics,
    light = unname(light),
...
#' @keywords internal
#' @noRd
.wf_report_blend <- function(w, audience) {
  ...
  if (audience == "manager") {
    ...
    action <- c(
      green = "publishable separately",
      yellow = "usable with caveats; inspect lambda sensitivity",
      red = "do not publish separately; one source lacks support"
    )[light]
    table <- cbind(
      table,
      light = unname(light),
...
.wf_report_markdown <- function(report) {
  chunks <- c(
    paste0("# ", report$title),
    "",
    paste0("Audience: ", report$audience),
    "",
    .wf_report_markdown_table(report$table)
  )
  if (length(report$sections) > 0) {
    for (section in names(report$sections)) {
      label <- gsub("_", " ", section, fixed = TRUE)
      chunks <- c(
        chunks,
        "",
        paste0("## ", label),
        "",
        .wf_report_markdown_table(report$sections[[section]])
      )
    }
  }
...
.wf_report_html <- function(report) {
  ...
  if (length(report$sections) > 0) {
    sections <- vapply(names(report$sections), function(section) {
      ...
      paste0(
        ...
        .wf_report_html_table(report$sections[[section]]),
        "</section>"
      )
    }, character(1))
  }
  paste0(
    ...
    "<p>Audience: ", .wf_report_html_escape(report$audience), "</p>",
    .wf_report_html_table(report$table),
...
#' @param lang Reserved language selector. Version 0.10 supports English only.
...
wf_report <- function(w, target = NULL,
  ...
  if (!is.null(lang) && !identical(lang, "en")) {
    wf_abort(
      "WFC 0.10 supports English reports only; output localization ships in 0.11.",
      "wf_error_input",
      list(lang = lang)
    )
  }
  ...
  if (inherits(w, "wf_weights")) {
    ...
    table <- if (audience == "manager") {
      .wf_report_manager_weights(diagnostics)
    } else {
      diagnostics
    }
    ...
    payload <- .wf_report_blend(w, audience)
    table <- payload$table
  ...
  report <- structure(
    list(
      title = "Weighting quality report",
      table = table,
      sections = sections,
      audience = audience,
      language = "en",
